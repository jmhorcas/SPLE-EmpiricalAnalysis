...
public  class  eVotingView  extends JPanel  implements eVotingInterface {
	...
	private String wellcomeMessage() {
	 ...
	 private void  authenticationOptions__wrappee__eVoting  () {
		authOptions = new JComboBox();
	}

	
	 private void  authenticationOptions__wrappee__votingKey  () {
		authenticationOptions__wrappee__eVoting();
		authOptions.addItem("votingKey");
	}

	
	 private void  authenticationOptions__wrappee__digitalCertificate  () {
		authenticationOptions__wrappee__votingKey();
		authOptions.addItem("digitalCertificate");
	}

	
	private void authenticationOptions() {
		authenticationOptions__wrappee__digitalCertificate();
		authOptions.addItem("fingerPrint");
	}

	
	
	 private void  encryptionOptions__wrappee__eVoting  () {
		encOptions = new JComboBox();
	}

	
	 private void  encryptionOptions__wrappee__DES  () {
		encryptionOptions__wrappee__eVoting();
		encOptions.addItem("DES");
	}

	
	private void encryptionOptions() {
		encryptionOptions__wrappee__DES();
