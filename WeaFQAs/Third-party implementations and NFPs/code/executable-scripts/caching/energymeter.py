...
POWER_KEY = "Power (W)"
CONSUMED_ENERGY_STDEV = "Consumed energy (Standard Deviation)"
...
def save_experiment_results(results, filename='energy_profile_samples.csv'):
	...
	with open(filename, 'w', newline='') as csvfile:
		...
		spamwriter.writerow([SAMPLE_KEY, PROCESS_KEY, COMPUTATIONAL_TIME_KEY, USAGE_CPU_KEY, CONSUMED_ENERGY_KEY, POWER_KEY, CONSUMED_ENERGY_STDEV])
		for r in results:
...
def get_energy_metrics_resume(results):
	...
	computational_time_median = statistics.mean(metrics[2])
	usage_cpu_median = statistics.mean(metrics[3])
	consumed_energy_median = statistics.mean(metrics[4])
	consumed_energy_stdev = statistics.pstdev(metrics[4])
	power_median = statistics.mean(metrics[5])

	return [metrics[1][0], computational_time_median, usage_cpu_median, consumed_energy_median, power_median, consumed_energy_stdev]
